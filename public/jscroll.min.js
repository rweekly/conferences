...
riveted.init({
  ...
  eventHandler: function(data) {
    _paq.push(['trackEvent', 'riveted', 'engaged time', data]);
  }
});
...
jQuery.scrollDepth({
  eventHandler: function(data) {
    if (data.event === "ScrollTiming"){
      _paq.push(['trackEvent', data.event, data.eventLabel, data.eventTiming]);
    } else {
      _paq.push(['trackEvent', data.event, data.eventAction, data.eventLabel]);
    }
  }
});
...
$('.content').jscroll({
    loadingHtml: "<small>Next Issue ... </small>",
    contentSelector: '#posts-content',
    nextSelector: '#next-page',
    loadingFunction: function(){
      var new_page = $('#next-page')[0].href.replace(/(https:\/\/)|(http:\/\/)|(\/index.html)/g, '');
      var xhr = new XMLHttpRequest();
      xhr.open("GET","https://page.rweekly.org/?page="+ new_page);
      xhr.send();
      _paq.push(['trackEvent', "jscroll", "new page", new_page]);
    },
    callback: function(){
      // NProgress.done();
      ...
      setTimeout(add_filter_func, 100);
...
var ga_uuid = function ga_uuidx(a) {
  return a ? (a ^ Math.random() * 16 >> a / 4).toString(16) :
      ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, ga_uuidx);
};
...
ga('set', dimensions.WINDOW_ID, ga_uuid());

ga(function(gaobj){
  var clientId = gaobj.get('clientId');
  gaobj.set(dimensions.CLIENT_ID, clientId);
  var originalBuildHitTask = gaobj.get('buildHitTask');
  gaobj.set('buildHitTask', function(model){
    model.set(dimensions.HIT_ID, ga_uuid(), true);
